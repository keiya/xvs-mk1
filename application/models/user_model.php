...
class User_model extends CI_Model {
	...
	public function create_history($uid,$video_id,$sojourn) {
		$res = $this->db->query(
			'SELECT duration FROM xvideos WHERE id=?'
		,array($video_id));
		$res_row = $res->row();
		$duration = $res_row->duration;
		...
		$svd_ratio = ($sojourn / $duration) + $svd_ratio_old;
		...
		$res = $this->db->query(
			...
			svd_ratio=?
			'
		,array($uid,$video_id,$svd_ratio,$uid,$video_id,$svd_ratio));
	}
